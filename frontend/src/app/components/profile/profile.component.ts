...
export class ProfileComponent implements OnInit {
  ...
  public OnAddFriendClicked(): void {
    this.usersService.addFriend(this.userDetails.id)
      .subscribe((response: HttpResponse) => { console.log(response);
        if (response.status === 201) {
            alert("Amis ajouté ");
        } else {
            alert("On peut pas ajouté l'amis !");
        }
      });
  }
